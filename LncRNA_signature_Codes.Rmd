---
title: "A lncRNA signature for metastasis prediction of NPC"
output: html_document
editor_options: 
  chunk_output_type: console
---

This is an R Markdown document for code we used in our study.
...
Patients characteristics in the training cohort (Table 1)
```{r,message = FALSE,warning = FALSE}
DataAll <- data_train
factorvars=c("AgeGroup","Sex","Tstage","Nstage","EBVDNAGroup","Metastasis","Progression","Death")
DataAll[factorvars]<-lapply(DataAll[factorvars],factor)
vars=c("AgeGroup","Sex","Tstage","Nstage","EBVDNAGroup","Metastasis","Progression","Death")
table1<-CreateTableOne(vars = vars,strata="lncRNA_signature",data=DataAll)
Res<-print(table1,showAllLevels = TRUE,
           cramVars = c("AgeGroup","Sex","Tstage","Nstage","EBVDNAGroup","Metastasis","Progression","Death"))
```
...
Patients characteristics in the internal validation cohort (Table 1)
```{r,message = FALSE,warning = FALSE}
DataAll_vd <- data_invd
factorvars=c("AgeGroup","Sex","Tstage","Nstage","EBVDNAGroup","Metastasis","Progression","Death")
DataAll_vd[factorvars]<-lapply(DataAll_vd[factorvars],factor)
vars=c("AgeGroup","Sex","Tstage","Nstage","EBVDNAGroup","Metastasis","Progression","Death")
table_vd<-CreateTableOne(vars = vars,strata="lncRNA_signature",data=DataAll_vd)
Res<-print(table_vd,showAllLevels = TRUE,
           cramVars = c("AgeGroup","Sex","Tstage","Nstage","EBVDNAGroup","Metastasis","Progression","Death"))
```
...
Patients characteristics in the external validation cohort (Table 1)
```{r,message = FALSE,warning = FALSE}
DataAll_gl <- Guilindata
factorvars=c("AgeGroup","Sex","Tstage","Nstage","Metastasis","Progression","Death")
DataAll_gl[factorvars]<-lapply(DataAll_gl[factorvars],factor)
vars=c("AgeGroup","Sex","Tstage","Nstage","Metastasis","Progression","Death")
table_gl<-CreateTableOne(vars = vars,strata="lncRNA_signature",data=DataAll_gl)
Res<-print(table_gl,showAllLevels = TRUE,
           cramVars = c("AgeGroup","Sex","Tstage","Nstage","Metastasis","Progression","Death"))
```

forest plot (Fig.3)\
- forestdata is the summary of univariate Cox analyses.
```{r,message = FALSE,warning = FALSE}
...
lncRNA function implication - hallmarks pathways(Fig.5A)
```{r,message = FALSE,warning = FALSE}
...
hallmark_sig <- hallmark_sig[order(hallmark_sig$Freq,decreasing = T),]
hallmark_sig[1:6,]
```
